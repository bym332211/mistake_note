...
> 原则：**摄像头默认关闭、学生手动开启**；**纯 CPU 可跑**；**先打通最小闭环**。
...
## 7. 错题卡数据模型与入库（SQLite + 媒体）（MVP）【未开始】
...
## 8. 基础相似题检索（仅 q_text_emb）（MVP）【未开始】
...
## 9. 一键 Demo 流（串联）（MVP 封板项）【未开始】

**目标**  
- 串起“开启摄像头 → OCR → 引导 → 入库 → 相似检索”。
...
## 14. 多视角向量与过滤（sol/concept）（M2）【未开始】
